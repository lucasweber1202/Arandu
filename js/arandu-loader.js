...
(function(){
  ...
  const loaded=[];
  const failed=[];
  function log(status,layer){const event={status,id:layer.id,src:layer.src,time:Date.now()};const list=JSON.parse(localStorage.getItem('arandu.loader.events')||'[]');list.push(event);localStorage.setItem('arandu.loader.events',JSON.stringify(list.slice(-220)))}
