...
(function(){
  ...
  const loaded=[];
  const failed=[];
  function log(status,layer){const event={status,id:layer.id,src:layer.src,time:Date.now()};const list=JSON.parse(localStorage.getItem('arandu.loader.events')||'[]');list.push(event);localStorage.setItem('arandu.loader.events',JSON.stringify(list.slice(-120)))}
  ...
  function run(){window.aranduLoader={version,layers,loaded,failed};document.body.classList.add('arandu-loader-ready');layers.forEach(addLayer);setTimeout(()=>{document.body.dataset.aranduLayersLoaded=String(loaded.length);document.body.dataset.aranduLayersFailed=String(failed.length)},1600)}
