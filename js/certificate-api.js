(function(){
  ...
  function view(target,cert){
    ...
    target.innerHTML='<span class="certificate-status">'+esc(cert.verification_status==='valid'?'Certificado válido':cert.verification_status||'valid')+'</span><h3>'+esc(title)+'</h3><p class="certificate-code">'+esc(cert.code)+'</p><div class="certificate-grid"><p><strong>Artista</strong><br>'+esc(name)+'</p><p><strong>Técnica</strong><br>'+esc(artwork.technique||payload.technique||'—')+'</p><p><strong>Dimensões</strong><br>'+esc(artwork.dimensions||payload.dimensions||'—')+'</p><p><strong>Edição</strong><br>'+esc(artwork.edition||payload.edition||'—')+'</p><p><strong>Ano</strong><br>'+esc(artwork.year||payload.year||'—')+'</p><p><strong>Emissão</strong><br>'+esc(issued)+'</p></div><p><strong>Observação:</strong> '+esc(cert.certificate_notes||payload.certificate_notes||'Registro verificado na base Arandu.')+'</p><div class="page-actions"><button class="button secondary" type="button" data-print-certificate>Imprimir validação</button><a class="cta secondary" href="autenticidade.html">Entender autenticidade</a></div>';
    return true;
  }
