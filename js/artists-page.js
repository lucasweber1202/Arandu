(function(){
  ...
  async function loadArtists(){try{const res=await fetch('/api/artists',{cache:'no-store'}); const data=await res.json().catch(()=>({})); if(res.ok&&Array.isArray(data.items)&&data.items.length) return data.items.map(normalizeArtist);}catch(e){} const res=await fetch('data/artists.json',{cache:'no-store'}); const data=await res.json(); return Array.isArray(data)?data.map(normalizeArtist):[];}
  async function loadWorks(){try{const res=await fetch('/api/catalog',{cache:'no-store'}); const data=await res.json().catch(()=>({})); if(res.ok&&Array.isArray(data.items)) return data.items.map(normalizeArtwork);}catch(e){} const res=await fetch('data/artworks.json',{cache:'no-store'}); const data=await res.json(); return Array.isArray(data)?data.map(normalizeArtwork):[];}
